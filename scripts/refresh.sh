...
for(i=2;i<=NF;i++)if($i!="+")print $i
	...
	for j in $(seq $screens); do
		choice=$(($(echo $j | dmenu -l 1 -m $((j - 1)) -p "Press Enter:")))
		if [ $choice -gt 0 ]; then
			mon=$((j))
			break
		fi
	done
	resolution=$(xrandr | awk -v var="$mon" -v count=0 '/.*\*.*/{count++; if(count==var)print $1}')
	response=$(($(xrandr | awk -v var="$mon" -v count=0 '/.*\*.*/{count++;
	if(count==var)
for(i=2;i<=NF;i++)if($i!="+")print $i
